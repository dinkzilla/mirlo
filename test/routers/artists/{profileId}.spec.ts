...
describe("artists", () => {
  ...
  describe("/{profileId}", () => {
    it("should GET /{profileId} with artist slug", async () => {
    ...
    it("should GET /{profileId} 404 when the artist is disabled", async () => {
    ...
    it("should GET /{profileId} 200 for an admin when the artist is disabled", async () => {
      const { user } = await createUser({ email: "artist@artist.com" });
      const profile = await createProfile(user.id, { enabled: false });
    ...
    it("should GET /{profileId} with wrong artist slug", async () => {
      const profileSlug = "test-artist";
      const user = await prisma.user.create({
        data: {
          email: "test@test.com",
